...
for (sampleID in sampleIDs){
  ...
  write_tsv(cyclone_tbl, paste0("data/cyclone/cyclone_", sampleID, ".tsv"))
}
...
sampleIDs = list.files("data/cyclone/", full.names=FALSE, pattern = "^cyclone_HAM*.*\\.tsv$") %>%
                    {gsub("^cyclone_", "", .)} %>%
                    {gsub(".tsv$", "", .)}

cyclone_full = read_tsv(paste0("data/cyclone/cyclone_", sampleIDs[1], ".tsv"))

for (i in 2:length(sampleIDs)){
  cyclone_sample = read_tsv(paste0("data/cyclone/cyclone_", sampleIDs[i], ".tsv"))
  cyclone_full = bind_rows(cyclone_full, cyclone_sample)
}

write_tsv(cyclone_full, "data/cyclone/cyclone_merged.tsv")
